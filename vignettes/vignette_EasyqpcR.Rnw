...
All the previous examples showed how to perform qPCR data analysis when all the
 ...
 to follow when we have too much samples to be contained in one run, thus when
...
As described in the work of Hellemans et al (2007) \cite{qbase}, the sample
 ...
 is no samples spreading over runs (all the samples for each gene are contained
 in a plate). 

Whereas in gene maximisation strategy the samples and genes are spread across
 runs. Thus, it is necessary to calibrate the NRQs with a run-specific and 
 gene-specific calibration factor.

We will now describe how to handle this last case. Follow these steps :
...
Do not worry about the NTC (no template control), they are not needed in qPCR
 data analysis (but they have to present a Ct difference > 5 Ct compared to the
 samples). Here, you can use the \code{gdfnotebook} function of the 
 \code{gWidgets} package.


After having removed the two aberrant values (RG2, Sample 15, Ct = 22.06916 ; 
 and RG3, Sample 18, Ct = 19.02328), rename the data frame as 
...
To respect the calculation of the NRQs which need the whole samples to take into
 ...
 on the whole samples but we will do it for each run with correction by 
 run-specific calibration factor and each time after inter-run variation 
 correction we isolate the corresponding CNRQs, for example:
...
We perform inter-run variation correction on the whole samples by the CF of the 
 ...
 the CF of the first is not the correct CF for the second run or any other run.
 Thus, we isolate (after inter-run variation correction on the whole samples by
 the CF of the first run) the samples concerned by this specific CF which are 
 the samples 1 to 4 and IRC 1.1, 2.1, 3.1. And we do it for each run-specific CF.
...
Note that in this case where the control samples are not present in all the 
 ...
 corrected by the calibration factor (we name it CNRQs). Thus, to have nicer 
 graphs, you will need to perform a scaling to your control group by doing a 
 geometrical mean of the CNRQs of your control samples and divide all the CNRQs 
...
CNRQs_scaled_to_group <- u/ctlgeom

yo <- CNRQs_scaled_to_group[c(1:4,8:11,15:18,22:25),]

yoyo <- colProds(yo)^(1/dim(yo)[1])
@
...
????? Why ctlgeomean is not = 1 ?

Aggregation of the NRQs, we isolate the NRQs corrected by its specific CF:
...
\begin{itemize}
	...
	\item{Samples 25 to 28 and IRC 1.7, 2.7, 3.1 are corrected by k}
    \item{Samples 29 to 32 and IRC 1.8, 2.8, 3.2 are corrected by l} 
\end{itemize}

Do not worry about nCTL, because in gene maximisation (or sample maximisation if 
 there are too many samples), the CTL are not present in all the runs, so you 
 will have to perform the scaling to control after.
